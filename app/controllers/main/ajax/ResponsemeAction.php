<?
class ResponsemeAction extends MyAction
{
  public function run()
  {
    ...
    if ($is_email) {
      ...
      Yii::app()->mailer->Subject = '[Григорий Васильков] Благодарю за отзыв';
      Yii::app()->mailer->Body = $controller->view('//main/email/response_user', $data);
      Yii::app()->mailer->AltBody = $controller->view('//main/email/response_user_text', $data);
    ...
    Yii::app()->mailer->Subject = '[Сайт Резюме] Зарегистрирован новый отзыв';
    Yii::app()->mailer->Body = $controller->view('//main/email/response_admin', $data);
    Yii::app()->mailer->AltBody = $controller->view('//main/email/response_admin_text', $data);
